...
export async function loadDatabase(): Promise<AppDatabase> {
  const db = getDatabaseSync(); // Get synchronous base/cache

  try {
    const { data: teamsData, error: teamsError } = await supabase.from('teams').select('*');
    if (!teamsError && teamsData) {
      const mergedTeams = teamsData.map(st => {
        const localTeam = db.teams.find(t => t.id === st.id);
        return localTeam ? { ...localTeam, name: st.name || localTeam.name } : st;
      });
      db.teams = [
        ...db.teams.filter(t => !mergedTeams.some((mt: any) => mt.id === t.id)),
        ...mergedTeams
      ] as any[];
    }
  } catch (err) {
  ...
  try {
    ...
    if (!usersError && usersData) {
      const mergedPersons = usersData.map(su => {
        const localPerson = db.persons.find(p => p.id === su.id);
        return localPerson ? { ...localPerson, name: su.name || localPerson.name } : su;
      });
      db.persons = [
        ...db.persons.filter(p => !mergedPersons.some((mp: any) => mp.id === p.id)),
        ...mergedPersons
      ] as any[];
    }
  } catch (err) {
  ...
  try {
    ...
    if (!leaguesError && leaguesData) {
      const mergedLeagues = leaguesData.map(sl => {
        const localLeague = db.leagues.find(l => l.id === sl.id);
        return localLeague ? { ...localLeague, name: sl.name || localLeague.name } : sl;
      });
      db.leagues = [
        ...db.leagues.filter(l => !mergedLeagues.some((ml: any) => ml.id === l.id)),
        ...mergedLeagues
      ] as any[];
    }
  } catch (err) {
    console.error('Error fetching leagues from Supabase', err);
  }

  try {
    const { data: gamesData, error: gamesError } = await supabase.from('games').select('*');
    if (!gamesError && gamesData) {
      const mergedGames = gamesData.map(sg => {
        const localMatch = db.matches.find(m => m.id === sg.id);
        // Fix mapping from snake_case to camelCase and default fallbacks
        return localMatch ? {
          ...localMatch,
          status: sg.status || localMatch.status,
          homeScore: sg.home_score !== undefined ? sg.home_score : localMatch.homeScore,
          awayScore: sg.away_score !== undefined ? sg.away_score : localMatch.awayScore,
          date: sg.date || localMatch.date
        } : { id: sg.id, status: sg.status, date: sg.date, homeTeamId: sg.home_team_id, awayTeamId: sg.away_team_id, homeScore: sg.home_score, awayScore: sg.away_score };
      });
      db.matches = [
        ...db.matches.filter(m => !mergedGames.some((mg: any) => mg.id === m.id)),
        ...mergedGames
      ] as any[];
    }
  } catch (err) {
