      ...
      <section ref={rinkContainerRef} className="relative bg-white mx-2 mt-2 mb-2 rounded-[40px] shrink-0 h-[280px] border-4 border-black shadow-lg overflow-hidden">
        ...
        >
          ...
          <div className="w-full h-full relative pointer-events-none">
            ...
            {/* Shot markers */}
            {events.filter(e => e.type === 'shot' && !e.isUndone && e.x !== undefined && e.y !== undefined).map(e => (
              <div
                key={e.id}
        ...
        {faceoffPopup && (
          ...
          >
            <button className="glossy-button px-4 py-2 rounded text-white font-bold text-xs" onClick={() => { onFaceoff('home'); setFaceoffPopup(null); setIsFaceoffMode(false); }}>
              {homeTeam || labels?.homeFallback || ''}
            </button>
            <button className="glossy-button px-4 py-2 rounded text-white font-bold text-xs" onClick={() => { onFaceoff('away'); setFaceoffPopup(null); setIsFaceoffMode(false); }}>
