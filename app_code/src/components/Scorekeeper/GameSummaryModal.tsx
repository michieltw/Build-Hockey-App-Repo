...
export default function GameSummaryModal({
  ...
  const handleExportCSV = async () => {
    ...
    if (location) csvContent += `${labels?.csvLocationLabel || ''},"${location}"\n`;
    if (competition) csvContent += `${labels?.csvCompetitionLabel || ''},"${competition}"\n`;
    if (matchType) csvContent += `${labels?.csvMatchTypeLabel || ''},"${matchType}"\n`;
    if (settings?.attendance) csvContent += `${labels?.csvAttendanceLabel || ''},${settings.attendance}\n`;
    if (officials && officials.length > 0) csvContent += `${labels?.csvOfficialsLabel || ''},"${officials.join(', ')}"\n`;
    if (linesmen && linesmen.length > 0) csvContent += `${labels?.csvLinesmenLabel || ''},"${linesmen.join(', ')}"\n`;
    csvContent += `\n`;
