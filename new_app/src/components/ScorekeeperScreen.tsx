...
import { createMatchEvent, saveMatch } from "../services/MatchService";
...
export default function ScorekeeperScreen({
    ...
    try {
      ...
      if (savedGame) {
        ...
        setGameState((prev) => ({
          ...
          timeRemaining:
            loadedConfig?.settings?.periodLength !== undefined
              ? loadedConfig.settings.periodLength
              : contract.defaultPeriodLength * 60,
          scoreHome:
            loadedConfig?.initialScoreHome !== undefined
              ? loadedConfig.initialScoreHome
              : contract.defaultInitialScoreHome,
          scoreAway:
            loadedConfig?.initialScoreAway !== undefined
              ? loadedConfig.initialScoreAway
              : contract.defaultInitialScoreAway,
          sogHome:
            loadedConfig?.initialSogHome !== undefined
              ? loadedConfig.initialSogHome
              : contract.defaultInitialSogHome,
          sogAway:
            loadedConfig?.initialSogAway !== undefined
              ? loadedConfig.initialSogAway
              : contract.defaultInitialSogAway,
          period:
            loadedConfig?.initialPeriod !== undefined
              ? loadedConfig.initialPeriod
  ...
  const [filter, setFilter] = useState<"all" | "shot" | "goal" | "penalty">(
    "all",
  );
  ...
  useEffect(() => {
    ...
    gameState.isRunning,
    config.settings?.penaltyClock,
    config.settings?.autoStopAtPeriodEnd,
  ]);
    ...
    setGameState((prev) => {
      ...
      if (!isContinuousPenalty && updatedPenalties.length > 0) {
        updatedPenalties = updatedPenalties
          .map((p) => ({
            ...p,
            secondsRemaining: p.secondsRemaining - prev.stoppageTime,
          }))
          .filter((p) => p.secondsRemaining > 0);
      }
      return {
  ...
  const handlePenaltySubmit = (data: {
    team: "home" | "away";
    player: string;
    reason: string;
    minutes: number;
  ...
  const handleFinishGame = async () => {
    try {
      // 1. Maak de match aan in Supabase
      const matchData = {
      ...
      const newMatches = await saveMatch(matchData);
      const matchId = newMatches[0]?.id;

      // 2. Loop door events en stuur ze naar de match_events tabel
      if (matchId) {
      ...
      showToast("Wedstrijd succesvol opgeslagen in database!");
    } catch (e) {
      console.error(e);
  ...
  const handleSaveGame = async () => {
    // Partial save kan ook via Supabase (bijv status: in_progress), voor nu is local storage een prima fallback
    try {
      localStorage.setItem(
        "blackout_hockey_saved_game",
        JSON.stringify(gameState),
      );
      showToast("Wedstrijd tussentijds opgeslagen (lokaal)!");
    } catch (e) {
      showToast("Opslaan mislukt");
    }
  };
    ...
    setGameState((prev) => {
      ...
      if (!isCurrentlyUndone) {
        ...
        if (eventToUndo.type === "shot") {
          if (eventToUndo.team === config.homeTeam)
            newSogHome = Math.max(0, newSogHome - 1);
          if (eventToUndo.team === config.awayTeam)
            newSogAway = Math.max(0, newSogAway - 1);
        } else if (eventToUndo.type === "goal") {
          if (eventToUndo.team === config.homeTeam)
            newScoreHome = Math.max(0, newScoreHome - 1);
          if (eventToUndo.team === config.awayTeam)
            newScoreAway = Math.max(0, newScoreAway - 1);
        } else if (eventToUndo.type === "penalty") {
          activePenalties = activePenalties.filter(
            (p) => p.eventId !== id && p.id !== id,
          );
        }
      } else {
        // We REDO this event
      ...
      const updatedEvents = prev.events.map((e) =>
        e.id === id ? { ...e, isUndone: !isCurrentlyUndone } : e,
      );
        ...
        onUpdateEvents={(newEvents) => {
          setGameState((prev) => {
            ...
            return {
              ...prev,
              events: newEvents,
              activePenalties: (prev.activePenalties || []).filter(
                (p) => !p.eventId || validIds.has(p.eventId),
              ),
            };
          });
        }}
