      ...
      <div className="overflow-y-auto flex-1 pb-10 relative z-0 scrollbar-none">
        {filteredEvents.map((ev) => {
          ...
          return (
            <div
              key={ev.id}
              className={`action-log-row flex items-center justify-between p-3 border-b border-[#333] text-sm ${isUndone ? "opacity-60 bg-black/20" : ""}`}
            >
              <div className="flex items-center gap-3 w-full">
                <span className="text-gray-400 font-mono w-16 text-xs">
                  {ev.time}
                </span>
                <div className="text-gray-500 w-2 text-center text-xs">|</div>
                <div
                  className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 border border-white/20"
                ...
                <span
                  className={`font-semibold flex-1 truncate ${isUndone ? "line-through text-gray-500" : "text-gray-200"}`}
                >
                  {ev.text}
              ...
              {!ev.isHistorical && (
                ...
                >
                  {isUndone ? (
                    <>
                      <RotateCw className="w-3 h-3 mb-0.5 text-amber-400" />{" "}
                      {labels?.redoButton || "REDO"}
                    </>
                  ) : (
                    <>
                      <RotateCcw className="w-3 h-3 mb-0.5" />{" "}
                      {labels?.undoButton || "UNDO"}
                    </>
                  )}
                </button>
              )}
