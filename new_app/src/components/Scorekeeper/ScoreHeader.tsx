...
export default function ScoreHeader({
  ...
  return (
    <header className="flex flex-col glossy-dark pt-4 pb-2 px-2 shrink-0 z-10 relative shadow-xl">
      <button
        onClick={onBack}
        className="absolute top-2 left-2 text-gray-400 hover:text-white z-20"
      >
      ...
      <div className="flex justify-between items-center relative z-10 pt-4 px-2">
          ...
          <div className="flex flex-col items-center">
            <div className="w-12 h-12 rounded-full flex items-center justify-center mb-1 overflow-hidden shrink-0 border-2 border-white/20 shadow-lg bg-[#222]">
              {homeLogo && !homeLogoError ? (
                ...
                >
                  <PawPrint
                    className="text-white w-7 h-7 drop-shadow-md"
                    fill="currentColor"
                  />
                </div>
              )}
            </div>
            <span className="font-bold text-lg" style={{ color: homeColor }}>
              {homeTeam}
            </span>
          </div>
          <div className="flex flex-col items-center">
        ...
        <div className="flex flex-col items-center justify-center w-1/3 relative">
          {!gameState.isRunning && gameState.stoppageTime > 0 && (
            <div className="absolute -top-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 z-20">
              <span className="text-gray-400 font-mono text-xs">
                {formatTime(gameState.stoppageTime)}
              </span>
              <button
          ...
          <div className="text-[10px] text-gray-400 font-bold tracking-wider cursor-pointer select-none">
            {gameState.period === 4
              ...
              : gameState.period >= 5
                ? "SO"
                : `${labels?.period || ""} ${gameState.period}`}
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 w-1/3">
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="font-bold text-lg" style={{ color: awayColor }}>
              {awayTeam}
            </span>
          </div>
        </div>
      ...
      <div className="flex justify-between mt-4 px-1 gap-2 text-xs">
        ...
        <div className="flex-1 bg-black/40 rounded p-1 flex flex-col border border-gray-700 min-h-[50px] max-h-28 overflow-y-auto">
          ...
          {!trackPenalties ? (
            <div className="text-center text-[10px] text-gray-500 py-1 font-mono italic">
              {labels?.penaltiesDisabled || ""}
            </div>
          ) : homePenalties.length === 0 ? (
            <div className="text-center text-[10px] text-gray-500 py-1 font-mono">
              {labels?.noPenalties || ""}
            </div>
          ) : (
            homePenalties.map((p) => (
              <div
                key={p.id}
                className="flex justify-between items-center px-1 font-mono py-0.5 border-b border-gray-800/40 last:border-0"
              >
                <span
                  className="text-gray-200 font-bold truncate max-w-[90px]"
                  title={p.player}
                >
                ...
                <div className="flex items-center gap-1">
                  <span className="text-yellow-400 font-bold text-[11px]">
                    {formatTime(p.secondsRemaining)}{" "}
                    <span className="text-[8px] text-gray-500">
                      {labels?.minutes || ""}
                    </span>
                  </span>
                  <Clock
                    className={`w-3 h-3 ${gameState.isRunning ? "text-yellow-400 animate-pulse" : "text-gray-500"}`}
                  />
                </div>
              </div>
            ))
