...
export default function GameSummaryModal({
  ...
  const handleExportCSV = async () => {
    ...
    let csvContent = `${labels?.csvHeaderDate || ""},${labels?.csvHeaderHome || ""},${labels?.csvHeaderAway || ""},${labels?.csvHeaderHomeScore || ""},${labels?.csvHeaderAwayScore || ""},${labels?.csvHeaderHomeSOG || ""},${labels?.csvHeaderAwaySOG || ""}\n`;
    csvContent += `"${now}","${homeTeam}","${awayTeam}",${gameState.scoreHome},${gameState.scoreAway},${gameState.sogHome},${gameState.sogAway}\n\n`;

    csvContent += `${labels?.csvGameDetailsTitle || ""}\n`;
    if (date) csvContent += `${labels?.csvDateLabel || ""},"${date}"\n`;
    if (time) csvContent += `${labels?.csvTimeLabel || ""},"${time}"\n`;
    if (location)
      csvContent += `${labels?.csvLocationLabel || ""},"${location}"\n`;
    if (competition)
      csvContent += `${labels?.csvCompetitionLabel || ""},"${competition}"\n`;
    if (matchType)
      csvContent += `${labels?.csvMatchTypeLabel || ""},"${matchType}"\n`;
    if (settings?.attendance)
      csvContent += `${labels?.csvAttendanceLabel || ""},${settings.attendance}\n`;
    if (officials && officials.length > 0)
      csvContent += `${labels?.csvOfficialsLabel || ""},"${officials.join(", ")}"\n`;
    if (linesmen && linesmen.length > 0)
      csvContent += `${labels?.csvLinesmenLabel || ""},"${linesmen.join(", ")}"\n`;
    csvContent += `\n`;

    csvContent += `${labels?.csvEventsHeaderTimestamp || ""},${labels?.csvEventsHeaderType || ""},${labels?.csvEventsHeaderTeam || ""},${labels?.csvEventsHeaderDesc || ""},${labels?.csvEventsHeaderX || ""},${labels?.csvEventsHeaderY || ""}\n`;
    ...
    activeEvents.forEach((e) => {
      ...
      csvContent += `"${e.time}","${e.type}","${e.team}","${cleanText}","${xVal}","${yVal}"\n`;
    });
          ...
          <div className="flex flex-wrap gap-4 bg-[#181818] rounded-xl p-3 text-xs font-mono text-gray-400 border border-[#2a2a2a]">
            ...
            {settings?.attendance ? (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {settings.attendance.toLocaleString()}{" "}
                  {labels?.attendanceSuffix || ""}
                </span>
              </div>
            ) : null}
            {competition && (
            ...
            <div className="bg-[#141414] border border-[#2a2a2a] rounded-xl divide-y divide-[#222] max-h-60 overflow-y-auto">
              {events.length === 0 ? (
                ...
                events.map((e) => (
                  <div
                    key={e.id}
                    className="p-2.5 flex items-center justify-between hover:bg-[#1a1a1a] transition-colors text-xs"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span
                        className={`font-semibold ${e.isUndone ? "line-through text-gray-500" : "text-gray-200"}`}
                      >
                        {e.text}
                      </span>
                    </div>
