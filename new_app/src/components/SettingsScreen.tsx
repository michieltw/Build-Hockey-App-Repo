...
const Section = ({
  ...
  return (
    <section className="flex flex-col gap-2">
      ...
      >
        <h2 className="font-mono text-[12px] font-bold text-tertiary tracking-widest uppercase">
          {title}
        </h2>
        {expanded ? (
...
const Row = ({
  ...
  <div
    className={`flex justify-between items-center py-2 ${border ? "border-b border-outline-variant/30" : ""} ${disabled ? "opacity-50 pointer-events-none" : ""}`}
  >
...
const Toggle = ({
  ...
  >
    <div
      className={`absolute top-1 w-4 h-4 rounded-full transition-all ${checked && !disabled ? "right-1 bg-black" : "left-1 bg-outline"}`}
    ></div>
  </button>
);
...
export default function SettingsScreen({
  ...
  const [homeTeam, setHomeTeam] = useState(
    scheduledGameData?.homeTeam || contract.defaultHomeTeam,
  );
  const [awayTeam, setAwayTeam] = useState(
    scheduledGameData?.awayTeam || contract.defaultAwayTeam,
  );
  ...
  const [homeRoster, setHomeRoster] = useState<Player[]>(
    scheduledGameData?.homeRoster || contract.defaultHomeRoster,
  );
  const [awayRoster, setAwayRoster] = useState<Player[]>(
    scheduledGameData?.awayRoster || contract.defaultAwayRoster,
  );

  const [initialScoreHome, setInitialScoreHome] = useState(
    contract.defaultInitialScoreHome,
  );
  const [initialScoreAway, setInitialScoreAway] = useState(
    contract.defaultInitialScoreAway,
  );
  const [initialSogHome, setInitialSogHome] = useState(
    contract.defaultInitialSogHome,
  );
  const [initialSogAway, setInitialSogAway] = useState(
    contract.defaultInitialSogAway,
  );
  const [initialPeriod, setInitialPeriod] = useState(
    contract.defaultInitialPeriod,
  );
  ...
  const [periodLength, setPeriodLength] = useState(
    contract.defaultPeriodLength,
  );
  const [p1Input, setP1Input] = useState(
    `${contract.defaultPeriodLength.toString().padStart(2, "0")}:00`,
  );
  const [trackIcing, setTrackIcing] = useState(contract.defaultTrackIcing);
  const [trackOffside, setTrackOffside] = useState(
    contract.defaultTrackOffside,
  );
  const [trackSOG, setTrackSOG] = useState(contract.defaultTrackSOG);

  const [officialGame, setOfficialGame] = useState(
    contract.defaultOfficialGame,
  );
  ...
  const [clockPauseBehavior, setClockPauseBehavior] = useState(
    contract.defaultClockPauseBehavior,
  );
  const [autoStopAtPeriodEnd, setAutoStopAtPeriodEnd] = useState(
    contract.defaultAutoStopAtPeriodEnd,
  );
  const [periodFormat, setPeriodFormat] = useState(
    contract.defaultPeriodFormat,
  );
  const [shootout, setShootout] = useState(contract.defaultShootout);
  const [soRules, setSoRules] = useState(contract.defaultSoRules);

  const [trackSOGLocation, setTrackSOGLocation] = useState(
    contract.defaultTrackSOGLocation,
  );
  const [trackFOW, setTrackFOW] = useState(contract.defaultTrackFOW);
  const [faceoffLocation, setFaceoffLocation] = useState(
    contract.defaultFaceoffLocation,
  );
  ...
  const [trackPenalties, setTrackPenalties] = useState(
    contract.defaultTrackPenalties,
  );
  const [penaltyClock, setPenaltyClock] = useState(
    contract.defaultPenaltyClock,
  );
  const [durationTypes, setDurationTypes] = useState(
    contract.defaultDurationTypes,
  );

  const [officialsMode, setOfficialsMode] = useState(
    scheduledGameData?.officials
      ? contract.customOfficialsMode
      : contract.defaultOfficialsMode,
  );
  const [linesmenMode, setLinesmenMode] = useState(
    scheduledGameData?.linesmen
      ? contract.customLinesmenMode
      : contract.defaultLinesmenMode,
  );
  const [venueMode, setVenueMode] = useState(
    scheduledGameData?.location
      ? contract.customVenueMode
      : contract.defaultVenueMode,
  );
  const [customVenue, setCustomVenue] = useState(
    scheduledGameData?.location || contract.defaultCustomVenue,
  );
  const [customOfficials, setCustomOfficials] = useState(
    scheduledGameData?.officials?.join(", ") || contract.defaultCustomOfficials,
  );
  const [customLinesmen, setCustomLinesmen] = useState(
    scheduledGameData?.linesmen?.join(", ") || contract.defaultCustomLinesmen,
  );
  ...
  useEffect(() => {
    try {
      const savedConfig = localStorage.getItem(
        "blackout_hockey_current_config",
      );
      if (savedConfig) {
        ...
        if (parsed.initialScoreHome !== undefined)
          setInitialScoreHome(parsed.initialScoreHome);
        if (parsed.initialScoreAway !== undefined)
          setInitialScoreAway(parsed.initialScoreAway);
        if (parsed.initialSogHome !== undefined)
          setInitialSogHome(parsed.initialSogHome);
        if (parsed.initialSogAway !== undefined)
          setInitialSogAway(parsed.initialSogAway);
        if (parsed.initialPeriod !== undefined)
          setInitialPeriod(parsed.initialPeriod);
        ...
        if (defaults) {
          ...
          if (defaults.trackIcing !== undefined)
            setTrackIcing(defaults.trackIcing);
          if (defaults.trackOffside !== undefined)
            setTrackOffside(defaults.trackOffside);
          if (defaults.trackSOG !== undefined) setTrackSOG(defaults.trackSOG);
          if (defaults.officialGame !== undefined)
            setOfficialGame(defaults.officialGame);
          if (defaults.gameType) setGameType(defaults.gameType);
          if (defaults.attendance !== undefined)
            setAttendance(defaults.attendance);
          if (defaults.ticketsSold !== undefined)
            setTicketsSold(defaults.ticketsSold);
          ...
          if (defaults.allowFillInPlayers !== undefined)
            setAllowFillInPlayers(defaults.allowFillInPlayers);

          if (defaults.gameClock !== undefined)
            setGameClock(defaults.gameClock);
          if (defaults.clockPauseBehavior)
            setClockPauseBehavior(defaults.clockPauseBehavior);
          if (defaults.autoStopAtPeriodEnd)
            setAutoStopAtPeriodEnd(defaults.autoStopAtPeriodEnd);
          ...
          if (defaults.trackSOGLocation !== undefined)
            setTrackSOGLocation(defaults.trackSOGLocation);
          if (defaults.trackFOW !== undefined) setTrackFOW(defaults.trackFOW);
          if (defaults.faceoffLocation !== undefined)
            setFaceoffLocation(defaults.faceoffLocation);
          if (defaults.goalscorer !== undefined)
            setGoalscorer(defaults.goalscorer);
          if (defaults.assists) setAssists(defaults.assists);

          if (defaults.trackPenalties !== undefined)
            setTrackPenalties(defaults.trackPenalties);
        ...
        if (defaultsStr) {
          ...
          if (defaults.periodLength !== undefined) {
            ...
            setP1Input(
              `${defaults.periodLength.toString().padStart(2, "0")}:00`,
            );
          }
          if (defaults.trackIcing !== undefined)
            setTrackIcing(defaults.trackIcing);
  ...
  const handleStart = () => {
    ...
    const finalHome =
      ...
      (teamSelection === contract.customTeamSelectionMode
        ? contract.fallbackHomeTeamCustom
        : contract.fallbackHomeTeamList);
    const finalAway =
      awayTeam.trim() ||
      (teamSelection === contract.customTeamSelectionMode
        ? contract.fallbackAwayTeamCustom
        : contract.fallbackAwayTeamList);
    ...
    const config: GameConfig = {
      ...
      location:
        venueMode === contract.customVenueMode ? customVenue : venueMode,
      competition:
        scheduledGameData?.competition || contract.defaultCustomVenue, // Lege fallback vanuit contract
      matchType: scheduledGameData?.matchType || contract.defaultCustomVenue,
      officials:
        officialsMode === contract.customOfficialsMode
          ? customOfficials.split(",").map((s) => s.trim())
          : [],
      linesmen:
        linesmenMode === contract.customLinesmenMode
          ? customLinesmen.split(",").map((s) => s.trim())
          : [],
      initialScoreHome,
      initialScoreAway,
      initialSogHome,
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <header className="bg-surface-container-low border-b border-outline-variant w-full top-0 flex items-center justify-between px-4 h-16 z-50 sticky">
        ...
        <h1 className="font-display text-[24px] font-bold text-primary tracking-tight">
          PRE-GAME SETTINGS
        </h1>
        <div className="w-10" />
      </header>
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 md:px-12 py-6 flex flex-col gap-10">
        ...
        <Section title="BEGINWAARDEN WEDSTRIJD" defaultExpanded={false}>
              ...
              <div className="flex gap-4 items-center bg-[#050505] border border-[#2A2A2A] rounded-lg p-3 justify-center">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setInitialScoreHome((prev) => Math.max(0, prev - 1))
                      }
                ...
                <span className="text-on-surface-variant font-display font-bold text-xl px-1">
                  -
                </span>
                <div className="flex flex-col items-center gap-1">
                  <span className="text-[11px] text-on-surface-variant font-semibold">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setInitialScoreAway((prev) => Math.max(0, prev - 1))
                      }
                      className="w-8 h-8 rounded bg-surface-container-high hover:bg-surface-container-highest flex items-center justify-center font-bold text-lg text-primary transition-colors select-none active:scale-90"
                    >
                      -
              ...
              <div className="flex gap-4 items-center bg-[#050505] border border-[#2A2A2A] rounded-lg p-3 justify-center">
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setInitialSogHome((prev) => Math.max(0, prev - 1))
                      }
                      className="w-8 h-8 rounded bg-surface-container-high hover:bg-surface-container-highest flex items-center justify-center font-bold text-lg text-primary transition-colors select-none active:scale-90"
                    >
                      -
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() =>
                        setInitialSogAway((prev) => Math.max(0, prev - 1))
                      }
                      className="w-8 h-8 rounded bg-surface-container-high hover:bg-surface-container-highest flex items-center justify-center font-bold text-lg text-primary transition-colors select-none active:scale-90"
                    >
                      -
          ...
          <Row label="Start Periode" border={false}>
            <Select
              ...
              value={
                contract.initialPeriodOptions[initialPeriod - 1] ||
                contract.initialPeriodOptions[0]
              }
              onChange={(e) => {
                const selectedIndex = contract.initialPeriodOptions.indexOf(
                  e.target.value,
                );
                setInitialPeriod(
                  selectedIndex !== -1
        ...
        <Section title="GAME CLOCK">
          <Row label="Game Clock">
            <Toggle
              checked={gameClock}
              onChange={() => setGameClock(!gameClock)}
            />
          </Row>
          ...
          <div
            className={`grid grid-cols-2 md:grid-cols-4 gap-4 py-2 ${!gameClock ? "opacity-50 pointer-events-none" : ""}`}
          >
        ...
        <Section title="TRACKING & STATS">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-2">
            <Row label="Icing">
              <Toggle
                checked={trackIcing}
                onChange={() => setTrackIcing(!trackIcing)}
              />
            </Row>
            <Row label="Offside">
              <Toggle
                checked={trackOffside}
                onChange={() => setTrackOffside(!trackOffside)}
              />
            </Row>
            <Row label="SOG">
              <Toggle
        ...
        <Section title="PENALTIES">
          <Row label="Penalties">
            <Toggle
              checked={trackPenalties}
              onChange={() => setTrackPenalties(!trackPenalties)}
            />
          </Row>
          <Row
            label="Penalty Clock"
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="bg-surface-container-low metallic-border rounded-lg p-4 inner-glow flex flex-col gap-2">
            <Row label="Local Backup" border={false}>
              <Toggle
                checked={localBackup}
